...
# Realiza una copia de seguridad de los directorios de inicio de los usuarios cuyo UID sea mayor que 999 en /backup en formato zip
rsync -az --delete /home/* "$backup_dir" 2>> "$logfile"
...
for user in $(cat /etc/passwd | awk -F: '$3 > 999 { print $1 }'); do
  if [ "$user" != "root" ]; then
    usermod --lock "$user"
    echo "Usuario $user bloqueado." >> "$logfile"
  fi
done
...
for user in $(cat /etc/passwd | awk -F: '$3 > 999 { print $1 }'); do
  if [ "$user" != "root" ]; then
    usermod --unlock "$user"
    echo "Usuario $user desbloqueado." >> "$logfile"
  fi
done
